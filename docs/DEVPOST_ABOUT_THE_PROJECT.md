...
- **Frontend Command Center**: Built with **Next.js 16 (App Router)**, **React 19**, **Tailwind CSS v4**, and **React Flow**. A local `LIVE_GEMINI` profile visualizes backend SSE activity; the public Firebase experience is an explicitly labeled, network-isolated `RECORDED_REPLAY` using sample data.
